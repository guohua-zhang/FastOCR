...
from Myocr.recognition import get_recognizer, get_text
from Myocr.utils import group_text_box, get_image_list, calculate_md5, get_paragraph,\
                   download_and_unzip, printProgressBar, diff, reformat_input,\
                   make_rotated_img_list, set_result_with_confidence,\
                   reformat_input_batched, merge_to_free
from Myocr.config import *
    ...
    def getDetectorPath(self, detect_network):
        if detect_network in self.support_detection_network:
            ...
            if self.detect_network == 'craft':
                from Myocr.detection import get_detector, get_textbox
            elif self.detect_network in ['dbnet18']:
                from Myocr.detection_db import get_detector, get_textbox
            else:
...
# write key information to excel
def postprocess(data, name):
...
def parse_args():
    ...
    parser.add_argument('--test_example_path', type=str, default='test_data', help='test_example_path.')
    parser.add_argument('--save_data_path', type=str, default='output', help='save_data_path.')
...
if __name__ == "__main__":
    ...
    test_data_path = args.test_example_path
    ...
    # model runner
    reader = Reader(['ch_sim','en']) # need to run only once to load model into memory
    
    with open(test_data_path + '/' + 'test_list.txt', 'r') as f:
        test_list = f.readlines()
    test_dataset = []
    for photo in test_list:
        test_dataset.append(test_data_path + '/' + photo.split()[0])
    # print(test_dataset)
    
    for adata in tqdm.tqdm(test_dataset):
        
        img = cv2.imread(adata)
        h, w, c = img.shape
        # print(h,w,c)
        result_information = cv2.cvtColor(np.array(Image.new('RGB', (w, h), (255, 255, 255))), cv2.COLOR_RGB2BGR)
        
        result = reader.readtext(adata)
        # print(result)
        ...
        for line in result:
            pts = line[0]
            information = line[1].replace(' ', '')
            # print(information)
            str_list.append(information)
            
            pts = np.array(pts, np.int32)
            cv2.polylines(img, [pts], True, (0, 0, 255), 2)
            
            result_information = cv2AddChineseText(result_information, information, (pts[0][0], pts[0][1]), (0, 0, 0), 20)

        canvas = np.zeros((h, w * 2, c), dtype=np.uint8)
        canvas[:, :w, :] = img
        canvas[:, w:, :] = result_information
        
        pre = save_data_path + '/' + adata.split('/')[1].split('.')[0]
        postprocess(result, pre)
        cv2.imwrite(pre + '_detection.png', img)
        cv2.imwrite(pre + '_recognition.png', result_information)
        cv2.imwrite(pre + '_comparion.png', canvas)